...
public function login($data)
    ...
    function edit_addon_record($data, $id)
	{
		$this->db->select('order_addons.*');
		$this->db->select('*');
		$this->db->from('order_addons');
		$this->db->where('id', $id);
		if($this->db->update('order_addons', $data)){
			return true;
		}
	}
...
// Insert addon data in database
public function addon_order_insert($data) 
{
...
{
	$this->db->select('order_addons.*, addon_services.title as subscription_plan_title, users.name as user_name, users.id as user_id, users.email as user_email, users.mobile as user_mobile, addon_services.price as subscription_price');
	$this->db->from('order_addons'); 
	...
	$this->db->order_by("order_addons.id", "asc");
	$query =  $this->db->get()->result_array();
	return $query;
}
